...
define AI_COMPILE_MODEL
	#demo7
	cd $(@D) && /usr/bin/python3 object_detect_opencv/data/od_onnx.py --quant_type uint8 --model ai_kmodel_data/model_file/object_detect/yolov5sFocus_320x3.onnx
	cp $(@D)/od.kmodel $(TARGET_DIR)/app/ai/object_detect_opencv/od_uint8.kmodel
	cd $(@D) && /usr/bin/python3 object_detect_opencv/data/od_onnx.py --quant_type bf16 --model ai_kmodel_data/model_file/object_detect/yolov5sFocus_320x3.onnx
	cp $(@D)/od.kmodel $(TARGET_DIR)/app/ai/object_detect_opencv/od_bf16.kmodel
	
	#demo1
	cd $(@D) && /usr/bin/python3 retinaface_mb_320_opencv/data/rf_onnx.py --quant_type uint8 --model ai_kmodel_data/model_file/retinaface/retinaface_mobile0.25_320.onnx
	cp $(@D)/rf.kmodel $(TARGET_DIR)/app/ai/retinaface_mb_320_opencv/rf_uint8.kmodel
	cd $(@D) && /usr/bin/python3 retinaface_mb_320_opencv/data/rf_onnx.py --quant_type bf16 --model ai_kmodel_data/model_file/retinaface/retinaface_mobile0.25_320.onnx
	cp $(@D)/rf.kmodel $(TARGET_DIR)/app/ai/retinaface_mb_320_opencv/rf_bf16.kmodel
